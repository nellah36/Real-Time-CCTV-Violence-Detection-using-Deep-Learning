...
class DataGenerator(Sequence):
    ...
    def load_data(self, path):
        ...
        if self.data_aug:
            ...
            # 💡 CRUCIAL FIX: Ensure frame size is correct after augmentations (e.g., 160x160)
            data = self.resize_frames(data)
            ...
            # For validation/testing (no data augmentation)
            if self.dataset == 'rwf2000' or self.dataset == 'surv':
                data = self.crop_center(data, x_crop=(320-224)//2, y_crop=(320-224)//2)  # center cropping only for test generators
            
            # 💡 CRUCIAL FIX: Ensure frame size is correct after center crop (e.g., 160x160)
            data = self.resize_frames(data)
